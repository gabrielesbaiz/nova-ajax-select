...
/**
 * Get the component key Nova uses to find the field during a sync.
 */
...
it('returns options scoped to the submitted parent', function (): void {
    $response = syncCreationField('city_id', ['region_id' => 2]);
...
it('returns no options while the parent is empty', function (): void {
    $response = syncCreationField('city_id', ['region_id' => null]);

    $response->assertOk()->assertJsonPath('options', []);
});

it('clears a value that no longer belongs to the parent', function (): void {
    $response = syncCreationField('city_id', ['region_id' => 2, 'city_id' => 1]);

    // Empty string, not null, so the browser clears the input and cascades.
    $response->assertOk()
...
it('keeps a value that is still valid', function (): void {
    syncCreationField('city_id', ['region_id' => 1, 'city_id' => 2])
        ->assertOk()
        ->assertJsonPath('value', 2);
});

it('resolves the third level of the chain', function (): void {
    $response = syncCreationField('zip_code', ['region_id' => 1, 'city_id' => 2]);
...
it('advertises the attributes it depends on', function (): void {
    syncCreationField('city_id', ['region_id' => 1])
        ->assertOk()
        ->assertJsonPath('dependsOn.region_id', 1);
});

it('serializes the parent values so an edit form can seed its first request', function (): void {
    syncCreationField('city_id', ['region_id' => 2])
        ->assertOk()
        ->assertJsonPath('ajaxSelect.parentValues.region_id', 2);
});
...
it('answers a search term sent through nova own sync request', function (): void {
    $response = test()->patchJson(
        ...
        ['region_id' => 1, AjaxSelectContext::SEARCH_KEY => 'Tar']
    );
...
it('never runs an options resolver for a guest', function (): void {
    ...
    $response = syncCreationField('city_id', ['region_id' => 1]);

    // Nova's middleware rejects an unauthenticated call before the field is reached.
...
it('syncs an update form against the stored model', function (): void {
    $customer = Customer::create(['region_id' => 1, 'city_id' => 1]);
